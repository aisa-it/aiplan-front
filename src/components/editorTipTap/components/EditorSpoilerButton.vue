...
const runCommand = (value: IStyleSpoiler) => {
  ...
  // Закрываем меню после применения (аналог v-close-popup у кнопки "Применить")
...
onMounted(() => {
  if (bus) {
    bus.on('focusSpoiler', setFocusedSpoilerPos as (...args: unknown[]) => void);
  }
});
onBeforeUnmount(() => {
  if (bus) {
    bus.off('focusSpoiler', setFocusedSpoilerPos as (...args: unknown[]) => void);
  }
});
</script>
