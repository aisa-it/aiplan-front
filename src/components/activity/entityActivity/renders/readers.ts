...
export function renderReader(m: HistoryEntry): string | undefined {
  const person =
        (m.new_value
  ...
  if (m.verb === 'added') {
    return `<span>добавил(-а) право на просмотр документа пользователю ${person}<span/>`;
  }
  if (m.verb === 'removed' || m.verb === 'remove') {
    return `<span>удалил(-а) право на просмотр документа пользователю ${person}<span/>`;
  }
}

export function renderReaderRole(m: HistoryEntry): string {
  const oldRole = valToRole(+m.old_value)?.label ?? m.old_value;
  const newRole = valToRole(+m.new_value)?.label ?? m.new_value;
  return `<span>изменил(-а) роль чтения документа с "${oldRole}" на "${newRole}"<span/>`;
}
