import { DtoIssue } from '@aisa-it/aiplan-api-ts/src/data-contracts';
import { CALENDAR_EVENT_TYPES } from '../constants/event-types';
import { CalendarEvent, CalendarEventType } from '../types/calendar';

export function mapIssueToCalendarEvents(
  issue: DtoIssue,
  type: CalendarEventType,
): CalendarEvent {
  if (type === 'target') {
    return createEvent(issue, 'target', issue.target_date ?? '');
  }

  if (type === 'completed') {
    return createEvent(issue, 'completed', issue.completed_at ?? '');
  }

  return createEvent(issue, 'created', issue.created_at ?? '');
}

function createEvent(
  issue: DtoIssue,
  type: keyof typeof CALENDAR_EVENT_TYPES,
  dateString: string,
): CalendarEvent {
  const config = CALENDAR_EVENT_TYPES[type];

  return {
    id: `${issue.id}:${type}`,
    type,

    date: new Date(dateString),
    color: config.color,

    issueData: {
      id: issue.id ?? '',
      issueUrl: issue.url,
      title: issue.name ?? '',
      identifier: issue.project_detail?.identifier ?? '',
      sequence_id: issue.sequence_id ?? 0,
      state_detail: issue.state_detail ?? {},
      completed_at: issue.completed_at ?? null,
      created_at: issue.created_at ?? '',
      target_date: issue.target_date ?? null,
    },
  };
}
