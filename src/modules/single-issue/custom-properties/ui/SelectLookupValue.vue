...
//services
import { getAvailablePropertyValues } from 'src/modules/single-issue/services/api';
...
const fetchRows = async (query?: string, isSearch?: boolean) => {
  ...
  try {
    ...
    countRows.value = res.rows?.count || 0;
    rows.value = [
      ...rows.value,
      ...(res.rows?.result || []).map((row) => ({
        label: row.value || '',
        value: row.id || '',
      })),
    ];
...
//lifecycle hooks
// строки грузим при открытии попапа (onPopupShow); тут — только сброс кэша
watch(
  () => props.templateId,
  () => {
    rows.value = [];
