  ...
  <q-dialog ref="dialogRef" @hide="handleClose">
    ...
    >
      <q-card-section class="column q-pt-none">
        ...
        />
        <DeleteFolderDialog
          v-if="folderForDelete?.id && folderForDelete.name"
          v-model="openDeleteFolder"
          :folder-id="folderForDelete?.id"
          :folder-name="folderForDelete?.name"
...
const props = defineProps<{
  sprint: DtoSprintLight | null | undefined;
}>();
...
const folderForDelete = ref<DtoSprintFolder | null>();
...
// const sprintsList = ref([] as DtoSprintFolder[]);
const loading = ref(false);

const sprintFolders = computed(() =>
  sprintsList.value?.filter((item) => item.id !== ROOT_FOLDER_ID),
);
...
const defaultFolderId = computed(() => {
  return sprintsList.value.find((folder) =>
...
const resetFoldersForEdition = () => {
  ...
  if (folderForDelete.value) {
    folderForDelete.value = null;
  }
};
...
watch(
  ...
  () => {
    sprintsList.value = sprintStore.sprintsList;
  },
);
